...
export default function Home() {
  ...
  return <main>
    ...
    <nav className="mobilenav" aria-label={t("nav.aria.mobile")}><button onClick={()=>nav("home")} className={page==="home"?"active":""}><span>⌂</span>{t("nav.home")}</button><button onClick={()=>nav("explore")} className={page==="explore"?"active":""}><span>⌕</span>{t("nav.explore")}</button><button onClick={()=>nav("scan")} className="scanmobile"><span>◎</span>{t("nav.scan")}</button><button onClick={()=>nav("water")} className={page==="water"?"active":""}><span>◍</span>{t("nav.water")}</button><button onClick={()=>nav("profile")} className={page==="profile"?"active":""}><span>○</span>{t("nav.profile")}</button></nav>
  </main>;
}
...
function HomePage({nav,query,setQuery,onSearch,onBarcode,onVoice,onVoiceInterim,busy}:{nav:(p:Page)=>void,query:string,setQuery:(s:string)=>void,onSearch:(v:string)=>void,onBarcode:(v:string)=>void,onVoice:(v:string)=>void,onVoiceInterim:(v:string)=>void,busy:boolean}) {
  ...
  <section className="storySection"><div className="sectionTitle"><div><span className="overline">{t("home.story.overline")}</span><h2>{t("home.story.title")}</h2></div><p>{t("home.story.body")}</p></div><div className="waterCards"><article className="waterCard green"><div className="drop">01</div><span>{t("water.green").toUpperCase()}</span><h3>{t("home.card.green.title")}</h3><p>{t("home.card.green.body")}</p><button onClick={()=>nav("learn")}>{t("home.card.more")} →</button></article><article className="waterCard blue"><div className="drop">02</div><span>{t("water.blue").toUpperCase()}</span><h3>{t("home.card.blue.title")}</h3><p>{t("home.card.blue.body")}</p><button onClick={()=>nav("learn")}>{t("home.card.more")} →</button></article><article className="waterCard grey"><div className="drop">03</div><span>{t("water.grey").toUpperCase()}</span><h3>{t("home.card.grey.title")}</h3><p>{t("home.card.grey.body")}</p><button onClick={()=>nav("learn")}>{t("home.card.more")} →</button></article></div></section>
  <section className="how"><span className="overline">{t("home.how.overline")}</span><h2>{t("home.how.title")}</h2><div className="steps"><div><b>1</b><span className="stepIcon">◎</span><h3>{t("nav.scan")}</h3><p>{t("home.how.step1body")}</p></div><i>→</i><div><b>2</b><span className="stepIcon">✓</span><h3>{t("home.how.step2")}</h3><p>{t("home.how.step2body")}</p></div><i>→</i><div><b>3</b><span className="stepIcon">≈</span><h3>{t("home.how.step3")}</h3><p>{t("home.how.step3body")}</p></div></div></section>
  <section className="insight"><div><span className="overline light">{t("home.insight.overline")}</span><h2>{t("home.insight.title")}</h2><p>{t("home.insight.body")}</p><div className="regionStats"><div><small>{t("home.insight.rain")}</small><b>92% <em>{t("home.insight.rainValue")}</em></b></div><div><small>{t("home.insight.stress")}</small><b>{t("home.insight.stressValue")}</b></div><div><small>{t("home.insight.season")}</small><b>{t("home.insight.seasonValue")}</b></div></div><button>{t("home.insight.cta")} →</button></div><div className="contour"><div className="pin">⌖</div><span>Maharashtra</span></div></section>
  <footer><div className="brand inverse"><img className="brandMark" src="/img/logo-mark.png" alt="" width={196} height={256}/><span>Jal<span>Drishti</span></span></div><p>{t("footer.disclaimer")}</p><div><button>{t("footer.methodology")}</button><button>{t("footer.sources")}</button><button>{t("footer.privacy")}</button></div></footer>
  </>}

function Scanner({state,setState,nav,onImage,onBarcode,setResolvedFromBridge,busy}:{state:"ready"|"scanning"|"confirm",setState:(s:"ready"|"scanning"|"confirm")=>void,nav:(p:Page)=>void,onImage:(b64:string,mt:string)=>void,onBarcode:(v:string)=>void,setResolvedFromBridge:(r:ResolveResult)=>void,busy:boolean}) {
  const {t}=useLang();
  return <section className="appPage scanPage"><div className="pageIntro"><span className="overline">{t("scan.overline")}</span><h1>{t("scan.title")}</h1><p>{t("scan.sub")}</p></div><div className="scannerLayout"><div className={`camera ${state}`}><LiveCamera busy={busy||state==="scanning"} onCapture={(b64,mt)=>{setState("scanning");onImage(b64,mt);setTimeout(()=>setState("ready"),400)}}/><div style={{marginTop:14}}><BarcodeInput onScan={onBarcode} label={t("scan.barcodeInstead")}/></div><PhoneBridge onCandidates={setResolvedFromBridge} onUseComputerCamera={()=>{}}/></div><aside className="scanHelp"><h3>{t("scan.tips")}</h3>{[["01",t("scan.tip1"),t("scan.tip1body")],["02",t("scan.tip2"),t("scan.tip2body")],["03",t("scan.tip3"),t("scan.tip3body")]].map(x=><div key={x[0]}><b>{x[0]}</b><p><strong>{x[1]}</strong><br/>{x[2]}</p></div>)}<button onClick={()=>nav("explore")}>{t("scan.manual")} →</button><small>{t("scan.supported")}</small></aside></div></section>}
